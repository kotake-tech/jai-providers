...
| `src/api.ts` | `/v1/models` の取得 |
| `src/cache.ts` | モデル id キャッシュの読み書き |
| `src/catalog.ts` | 同梱のモデルメタデータと表示名の生成 |
| `src/credentials.ts` | `auth.json` と環境変数からの資格情報解決 |
| `src/discovery.ts` | キャッシュ・API・カタログのフォールバック |
| `src/deep-think.ts` | `deep_think` ツールとシステムプロンプト |
